...
import pickle, time, re, threading, json
import socket as sk
...
class GameField:
    ...
    playerSymbolsReverse = {"x":-1,"o":1, "□":0}
    beginningPlayer = -1
...
class RemotePlayer(Player):
    ...
    def setGameField(self, gameField:GameField):
        self.gameField = gameField
    ...
    def move(self):
        ...
        clientSocket.send(json.dumps({"board":self.gameField.board.tolist(), "role":"attacker" if self.playerIndex == -1 else "defender"}).encode()) # send data to server
        
        response = clientSocket.recv(2048).decode()

        clientSocket.close()

        serversMove = json.loads(response)
        self.gameField.makeMove(self, [serversMove["x"],serversMove["y"]])
...
if __name__ == "__main__":
    ...
    game = GameField([attacker, defender], [5,5], useLog = False, enableRandomness = True, winBy = 4)
    attacker.load()
    defender.load()

    game.startRowOfGames(5000, 1)
